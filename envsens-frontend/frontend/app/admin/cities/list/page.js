...
const CitiesList = () => {
    ...
    useEffect(() => {
        if (userHasPermitTo(permissions, 36) || profile.is_superuser) {
            if (cities.length === 0 || loaded === false) {
                getCities();
                setLoaded(true);
            }
        } else {
    ...
    const handleDeleteCity = async () => {
        try {
            await axios.delete(`/cities/delete/${selectedCity.id}/`, {
                headers: {
                    'Authorization': `Bearer ${token.access}`
                }
            });
    ...
    const header = (
        <div style={{ backgroundColor: "#FFFAFA" }}>
            <div className={styles.container_header}>
                <p className={styles.titre}><FaChevronRight /> Liste de toutes les villes</p>
            </div>
            <div className="table-header" style={{ display: "flex", flexDirection: "row", alignItems: "center", alignContent: "center" }}>
                ...
                <Button label="Ajouter" icon="pi pi-plus" className="p-button-text" onClick={() => { if (userHasPermitTo(permissions, 33) || profile.is_superuser) { router.push("/admin/cities/") } }} style={{ backgroundColor: "#E6E6E6", borderRadius: "13px", color: "#828282" }} />
            </div>
        </div>
    );
    ...
    const actionBodyTemplate = (rowData) => {
        const menuItems = [
            { label: 'Voir Details', command: () => { if (userHasPermitTo(permissions, 71) || profile.is_superuser) { router.push("/admin/cities/" + rowData.id) } else { setShowNotPermitModal(true) } }},
            { label: 'Modifier', command: () => { if (userHasPermitTo(permissions, 34) || profile.is_superuser) { router.push("/admin/cities/update/" + rowData.id) } else { setShowNotPermitModal(true) } }},
            { label: 'Supprimer', command: () => { if (userHasPermitTo(permissions, 35) || profile.is_superuser) { setSelectedCity(rowData); setShowDeleteModal(true); } else { setShowNotPermitModal(true) } }},
        ];
    ...
    return (
        <>
            ...
            <div className="p-datatable p-component" style={{ height: '100vh' }}>
                <DataTable globalFilter={globalFilter} rowsPerPageOptions={[5, 10, 25, 50]} value={cities} paginator rows={8} header={header} className="p-datatable-gridlines" style={{ height: 'calc(100vh - 145px)' }}>
            ...
            <Dialog header="Confirmation de suppression" visible={showDeleteModal} style={{ width: '30vw' }} modal footer={
                <div className='btn_div'>
                    ...
                    <Button className={styles.button} label="Oui" icon="pi pi-check" onClick={handleDeleteCity} autoFocus />
                </div>
