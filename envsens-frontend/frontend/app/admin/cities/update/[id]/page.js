...
const CreateCity = () => {
    ...
    const fetchCityData = async (groupId) => {
        await axios.get(`/city/details/${groupId}/`, {
            headers: {
                "Authorization": "Bearer " + token.access
            }
        }).then((response) => {
    ...
    const onSubmit = async (data) => {
        data['country'] = selectedCountry ? selectedCountry : cityData.country;
        try {
            await axios.put(`/cities/update/${id}/`, data, {
                headers: {
                    'Authorization': `Bearer ${token.access}`
                }
            });
            ...
            toast.current.show({ severity: 'error', summary: 'Non !!', detail: 'Vous n\'avez pas l\'autorisation de modifier une ville.', life: 3000 });
    ...
    return (
        <div>
            <div className={styles.container_header}>
                <Toast ref={toast} />
                <p className={styles.titre}><FaChevronRight /> Mise à jour de la Ville : {cityData?.label}</p>
            </div>
