...
const NetworksList = () => {
    ...
    const header = (
        <div style={{ backgroundColor: "#FFFAFA" }}>
            ...
            <div className="table-header" style={{ display: "flex", flexDirection: "row", alignItems: "center", alignContent: "center" }}>
                ...
                <Button label="Ajouter" icon="pi pi-plus" className="p-button-text" onClick={() => { if (userHasPermitTo(permissions, 37) || profile.is_superuser) { router.push("/admin/networks/") } }} style={{ backgroundColor: "#E6E6E6", borderRadius: "13px", color: "#828282" }} />
            </div>
        </div>
    );
    ...
    const actionBodyTemplate = (rowData) => {
        const menuItems = [
            ...
            { label: 'Modifier', command: () => { if (userHasPermitTo(permissions, 38) || profile.is_superuser) { router.push("/admin/networks/update/" + rowData.id) } else { setShowNotPermitModal(true) } }},
            { label: 'Supprimer', command: () => { if (userHasPermitTo(permissions, 39) || profile.is_superuser) { setSelectedNetwork(rowData); setShowDeleteModal(true); } else { setShowNotPermitModal(true) } }},
        ];
        ...
        <>
            ...
            <div className="p-datatable p-component" style={{ height: '100vh' }}>
                <DataTable globalFilter={globalFilter} rowsPerPageOptions={[5, 10, 25, 50]} value={networks} paginator rows={8} header={header} className="p-datatable-gridlines" style={{ height: 'calc(100vh - 145px)' }}>
                    ...
                    <Column field="num_devices" header="Capteurs" sortable />
                    <Column field="num_gateways" header="Passerelles" sortable />
