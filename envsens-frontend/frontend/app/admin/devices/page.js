...
const schema = yup.object().shape({
    ...
    is_mobile: yup.string().required("Is Mobile est requis"),
    network: yup.string().required("Network est requis"),
    gateway: yup.string().required("Gateway est requis"),
...
const CreateDevice = () => {
    ...
    const onSubmitForm = async (data) => {
        ...
        try {
            await axios.post('/devices/create/', data, {
                headers: {
                    'Authorization': `Bearer ${token.access}`
                }
            });
            toast.current.show({ severity: 'success', summary: 'Succès', detail: 'Device créé avec succès', life: 3000 });
            // Redirection vers la liste des devices après le succès de la création
            ...
            if (error?.response?.status === 401 && error?.response?.data?.error === 'Unauthorized') {
                toast.current.show({ severity: 'error', summary: 'Non !!', detail: 'Vous n\'avez pas l\'autorisation de créer un device.', life: 3000 });
            } else {
                toast.current.show({ severity: 'error', summary: 'Erreur', detail: 'Erreur lors de la création du device', life: 3000 });
            }
        }
    };
    ...
    useEffect(() => {
        fetchNetworks();
        if (!(userHasPermitTo(permissions, 55) || profile.is_superuser)) {  // Assurez-vous d'avoir la bonne permission ID
    ...
    return (
        <div className={styles.pageContainer}>
            <div className={styles.container_header}>
                <Toast ref={toast} />
                <p className={styles.titre}><FaChevronRight /> Ajout d'un Device :</p>
            </div>
            <div className={styles.container}>
                <h2 className={styles.title}>Veuillez entrer les informations du nouveau device</h2>
                <form onSubmit={handleSubmit(onSubmitForm)} className={styles.form}>
                    ...
                    <div className={styles.formGroup}>
                        <Dropdown id="gateway" {...register('gateway')} value={selectedGateway} onChange={(e) => { setSelectedGateway(e.target.value); setValue('gateway', e.target.value); }} placeholder="Gateway..." options={gateways} optionLabel="description" optionValue="id" className={styles.input} />
                    ...
                    <div className={styles.formGroup}>
                        <InputTextarea id="description de capteur" {...register('description')} placeholder="Description..." className={styles.textarea} />
